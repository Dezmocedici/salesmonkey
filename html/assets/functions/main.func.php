...
function getAlert()
{
  if(isset($_GET['alert']))
  {
    ...
    if ($message === 'invalid-sql')
    {
      echo '<script type="text/javascript" language="javascript"> swal("' . $message .'") </script>';
    }
  }
}
...
function login($conn)
{
  if (isset($_POST['login_btn']))
  {
    ...
    {
      header('LOCATION: http://' . $_SERVER['SERVER_NAME'] . '/active.php');
      exit();
    }
    ...
    {
      header("Location: ../dashboard.php?login-successed");
      exit();
    }
...
function sendLink($conn, $type, $email = null)// 2 types of verification mail: activateAccount | resetPassword
{
